...
| 2026.9.8  | 1.指纹、目录扫描的目标中无协议头导致失败优化 @7eleven  2.端口扫描导出失败优化 @7eleven  3.代理池的测绘引擎的状态与配置界面的状态不同步优化@honest_run 代理池批量添加优化 @111  4.小程序功能查看源文件里面增加搜索高亮，上一个，下一个查询 @延延  5.代理池在项目里面切换项目状态失效优化 @1Z1  6.JsFinder的解析接口进一步优化 @雪山乘客  7.windows环境下偶有白色边框的问题 @Bonfire @111 @walkingdone  8.heapdump的分析结果增加搜索、高亮显示 @Hide  9.hunter测绘查询URLBase64的编码问题bug @开心~ |
| --------- | ------------------------------------------------------------ |
