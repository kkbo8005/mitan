...
**工  具  箱**：Sessionkey,heapdump, 微信小程序，密码本，随机用户，社工字典，杀软识别，编码解码，数据处理，反弹 Shell，项目资产分拣
...
![5f597187-69c1-4210-b563-d416eee621b5](Readme.assets/5f597187-69c1-4210-b563-d416eee621b5.png)
...
​    **（6） MCP server - 43个工具赋能 Agent**

密探2.0 的MCPServer提供包含常规功能，项目相关，主体查询，测绘引擎，fuzz爆破，云安全，其他等39个MCP工具，可以为其他agent智能体提供交互，交互结果也会继续保存到项目的历史记录中，便于对各项工作过程数据进行统一管理。

![image-20260915120402928](/Volumes/soft/网络安全/github_mitan/Readme.assets/image-20260915120402928.png)

![2e3ac244-83cd-4b70-8e91-713e81207dff](Readme.assets/2e3ac244-83cd-4b70-8e91-713e81207dff.png)

​    **（7） 项目数据资产分拣功能**

![3039b543-b039-42c1-86cf-d9a20ba6f4c3](Readme.assets/3039b543-b039-42c1-86cf-d9a20ba6f4c3.png)
...
![15d43c4c-95be-4f5e-8cc9-9fddda025160](Readme.assets/15d43c4c-95be-4f5e-8cc9-9fddda025160.png)

如果在局域网上使用，需要对密探所属机器的命令执行，文件删除等危险操作进行限制，需要进行确认才能操作。
...
| 2026.9.22 | 1.优化poc生成的yaml编辑器  2.优化微信小程序的api路径提取算法，优化小程序列表  3.优化linux版本下的appImage启动问题 @FX42S  4.poc扫描的yaml文件识别优化  5.windows架构下偶尔有启动卡进度的bug @Hui哥  6.优化带筛选的列表头筛选时高亮显示，优化筛选结果的复制 @Miraitowa  7.端口扫描模块的右键菜单增加联动弱口令扫描功能 @Miraitowa |
| --------- | ------------------------------------------------------------ |
| 2026.9.15 | 1.poc模块的excel宽度受限导出bug @H   2.windows版本的边框与四角缩放bug @天亮   3.Claude Code的mcp的工具数量识别优化 @索隆不睡觉   4.适配windows环境下最大化，最小化，关闭按钮 @小机智 @mod. @lucky·star   5.优化密码破解模块的界面队列进度及密码过滤优化 @🍑桃子🍑   6.在密码破解模块中增加oracle服务 @济川   7.工具箱模块增加资产分拣功能 @狮嗯   8.子域名爆破的界面端口改为自定义模式 @狮嗯   9.项目列表的增加进入项目图标按钮   10.主体查询，ICP查询增加ICP_Query,镜芯API @狮嗯 |
| 2026.9.8  | 1.指纹、目录扫描的目标中无协议头导致失败优化 @7eleven  2.端口扫描导出失败优化 @7eleven  3.代理池的测绘引擎的状态与配置界面的状态不同步优化 @小机智 代理池批量添加优化 @111  4.小程序功能查看源文件里面增加搜索高亮，上一个，下一个查询 @延延  5.代理池在项目里面切换项目状态失效优化 @1Z1  6.JsFinder的解析接口进一步优化 @雪山乘客  7.windows环境下偶有白色边框的问题 @Bonfire @111 @walkingdone  8.heapdump的分析结果增加搜索、高亮显示 @Hide  9.hunter测绘查询URLBase64的编码问题bug @开心~ |
| 2026.8.29 | 1.优化项目文件可自定义目录 @根号三 @小华 @TyC2l& @TL @延延 @鸭鸭 @R. @两斤土豆 @千阳 @Xurany 2. 增加对chunk懒加载js,js.map的处理，优化jsFinder @雪山乘客 @汉堡王  3.对端口扫描功能进行优化提升识别覆盖率 @🐰🐰🐰  4.配置代理与首界面的代理不一致 @coke  5.代理池的免费代理抓取优化 @Mix @YY 6.POC测试增加可以代理的优化 @氼乚  7.jwt密钥破解bug优化 @🐟  8.mcp工具调用日志 @千阳  9. 文本框的左右键存在添加字符的bug @Mix  10.MCP的列表页面的自带mcp测试bug优化 @是业不是叶  11.MCP设置界面对token设置与否进行优化 @0xc3  12.地图密钥增加google地图apikey验证 @伊诺  13.代理配置界面与主界面的代理开启的不同步优化 @고양이 |
| 2026.8.23 | 1.优化项目文件可自定义目录 @根号三 @小华 @TyC2l& @TL @延延 @鸭鸭 @R. @两斤土豆 @千阳  2. 增加对chunk懒加载js,js.map的处理，优化jsFinder @雪山乘客 @汉堡王  3.对端口扫描功能进行优化提升识别覆盖率 @🐰🐰🐰  4.配置代理与首界面的代理不一致 @coke  5.代理池的免费代理抓取优化 @Mix  6.POC测试增加可以代理的优化 @氼乚  7.jwt密钥破解bug优化 @🐟  8.mcp工具调用日志 @千阳  9. 文本框的左右键存在添加字符的bug @Mix |
| 2026.8.16 | 重构密探功能，优化用户体验，根据渗透测试需求，重新调整工具导航,感谢以下师傅提供数据及账号支持！@曾哥 @温酒看日出 @🌲 @99977 @至明善渊 @ZoomEye官方  @过去式 @梧祁@小浪 @REMADOME.md @Forget me @Sukalis  @重剑无锋   @大白哥  @湫东坡_up  @小机智 @醉殷 @TyC2l& @陈明 @奔奔 @狮恩 |
