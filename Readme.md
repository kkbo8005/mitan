...
| 2025.2.22 | 增加了代理池功能，开启本地监听端口，密探扫描模块或其他软件可以利用本地监听端口使用代理池，当前支持HTTP,SOCKS5两种协议（先来个测试版，继续完善）。 |
| --------- | ------------------------------------------------------------ |
| 2025.1.17 | 修复了信息查询的端口记录（fofa旧的api调用不了，新的端口开放记录需要配置fofa key才能查询），修复法定代表人和注册资金查询，修复了风鸟数据查询的数据返回格式变化的问题。 |
...
![image-20250113164854550](Readme.assets/image-20250113164854550.png)
